...
public class NativeGitIncludedFilesRepository {

  private static final Logger LOG = Loggers.get(NativeGitIncludedFilesRepository.class);
  private final Set<Path> includedFiles = new HashSet<>();

  public NativeGitIncludedFilesRepository(Path baseDir) {
    indexFiles(baseDir);
  ...
  private void indexFiles(Path baseDir) {
    try {
      Process gitLsFiles = NativeGitUtils.getVerifiedProcessBuilder(baseDir).command(NativeGitUtils.getGitExecutable(), "ls-files", "-c", "-o", "--exclude-standard").start();
