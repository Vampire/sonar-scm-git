...
public class NativeGitUtils {
  ...
  private static final Logger LOG = Loggers.get(NativeGitUtils.class);

  public static final String GIT_EXECUTABLE_DEFAULT = "git";

  public static final String GIT_EXECUTABLE_PROPERTY = "sonar.git.executable";
  ...
  public static void determineGitExecutable(Configuration configuration) {
    if ((configuration != null) && configuration.hasKey(GIT_EXECUTABLE_PROPERTY)) {
      ...
      if (file.exists()) {
        ...
        NativeGitUtils.gitExecutable = gitExecutable;
      } else {
      ...
      NativeGitUtils.gitExecutable = GIT_EXECUTABLE_DEFAULT;
    }
  }

  static String getGitExecutable() {
    String gitExecutable = requireNonNull(NativeGitUtils.gitExecutable, "Call determineGitExecutable first");
    if (gitExecutableVerified.compareAndSet(false, true)) {
      if (!verifyGitExecutable(gitExecutable)) {
  ...
  public static boolean verifyGitExecutable(String gitExecutable) {
    try {
      if (new ProcessBuilder(gitExecutable, "--version").start().waitFor() == 0) {
        return true;
      }
    } catch (IOException e) {
      LOG.info("Failed to invoke native git executable", e);
  ...
  static ProcessBuilder getVerifiedProcessBuilder(Path basedir) {
    ...
    try {
      Process gitRevParse = processBuilder.command(NativeGitUtils.getGitExecutable(), "rev-parse", "--is-inside-work-tree").start();
  ...
  static Path getRelativePathFromScmRoot(Path path) {
    try {
      ...
      Process gitRevParse = getVerifiedProcessBuilder(pathDirectory.toPath()).command(NativeGitUtils.getGitExecutable(), "rev-parse", "--show-toplevel").start();
      try (BufferedReader stdout = new BufferedReader(new InputStreamReader(gitRevParse.getInputStream()))) {
