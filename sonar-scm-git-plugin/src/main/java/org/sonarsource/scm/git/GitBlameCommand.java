...
public class GitBlameCommand extends BlameCommand {
  ...
  public void blame(BlameInput input, BlameOutput output) {
    ...
    try {
      Process gitRevParse = GitUtils.getVerifiedProcessBuilder(baseDir.toPath()).command(GitUtils.getGitExecutable(), "rev-parse", "--is-shallow-repository").start();
  ...
  private void blame(BlameOutput output, File baseDir, InputFile inputFile) {
    ...
    try {
      ...
      Process gitBlame = GitUtils.getVerifiedProcessBuilder(baseDir.toPath()).command(GitUtils.getGitExecutable(), "blame", "--incremental", "-w", "--", relativeFilePath).start();
