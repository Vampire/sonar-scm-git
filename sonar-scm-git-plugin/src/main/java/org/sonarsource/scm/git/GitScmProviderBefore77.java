...
public class GitScmProviderBefore77 extends ScmProvider {
  ...
  public boolean supports(File baseDir) {
    try {
      Process gitRevParse = new ProcessBuilder(GitUtils.getGitExecutable(), "rev-parse", "--is-inside-work-tree").directory(baseDir).start();
  ...
  public Set<Path> branchChangedFiles(String targetBranchName, Path rootBaseDir) {
    ProcessBuilder processBuilder = GitUtils.getVerifiedProcessBuilder(rootBaseDir);
    ...
    try {
      Process gitDiff = processBuilder.command(GitUtils.getGitExecutable(), "-c", "core.quotepath=false", "diff", "--name-only", "--relative", "--no-renames", "--diff-filter=AM", targetRef + "...").start();
  ...
  public Map<Path, Set<Integer>> branchChangedLines(String targetBranchName, Path projectBaseDir, Set<Path> changedFiles) {
    ProcessBuilder processBuilder = GitUtils.getVerifiedProcessBuilder(projectBaseDir);
    String targetRef = resolveTargetRef(targetBranchName, processBuilder);
    if (targetRef == null) {
      return null;
    }
    ...
    for (Path path : changedFiles) {
      ...
      try {
        ...
        Process gitDiff = processBuilder.command(GitUtils.getGitExecutable(), "diff", "-w", targetRef , "--", relativeFilePath).start();
  ...
  private String resolveTargetRef(String targetBranchName, ProcessBuilder processBuilder) {
    try {
      ...
      Process gitRevParse = processBuilder.command(GitUtils.getGitExecutable(), "rev-parse", "--quiet", "--verify", targetRef).start();
      if (gitRevParse.waitFor() == 0) {
        return targetRef;
      }
      targetRef = "refs/remotes/origin/" + targetBranchName;
      gitRevParse = processBuilder.command(GitUtils.getGitExecutable(), "rev-parse", "--quiet", "--verify", targetRef).start();
      if (gitRevParse.waitFor() == 0) {
        return targetRef;
      }
    } catch (IOException e) {
  ...
  public Path relativePathFromScmRoot(Path path) {
    return GitUtils.getRelativePathFromScmRoot(path);
  }
  ...
  public String revisionId(Path path) {
    try {
      ...
      Process gitRevParse = GitUtils.getVerifiedProcessBuilder(pathDirectory.toPath()).command(GitUtils.getGitExecutable(), "rev-parse", "--quiet", "--verify", "HEAD").start();
      try (BufferedReader stdout = new BufferedReader(new InputStreamReader(gitRevParse.getInputStream()))) {
